...
- Subtoolbar plugins register their toggle with `window.registerSubtoolbar(btn)` and open/close via `window.openSubtoolbar(bar, btn)` — one options bar visible at a time. Right-panel plugins still need a small hide-list entry in `app.js` `openRightPanel()` (the one remaining core touchpoint).
