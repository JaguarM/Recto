...
(function () {
  ...
  // Spans (embedded or OCR words) sharing the box's row, by vertical overlap.
  function spansOnRow(box, type) {
    return utbState.boxes.filter((b) => {
      if (b.page !== box.page) return false;
      if (type ? b.type !== type : b.type !== 'embedded' && b.type !== 'ocr') return false;
      const overlap = Math.min(box.y + box.h, b.y + b.h) - Math.max(box.y, b.y);
      return overlap >= box.h * 0.5;
    });
  }

  // The text-line spans the box is measured against.
  //
  // OCR words are preferred over embedded ones whenever OCR has read this row:
  // OCR reads the glyphs actually VISIBLE on the page after redaction, whereas
  // the embedded text layer can still carry glyphs the redaction removed from
  // view. A following word like "and" whose leading "a" was dropped from the
  // text layer survives there as "nd" positioned at the "n" — ~one glyph to the
  // right of where the word visibly begins. Measuring the box edge against that
  // clipped neighbour pushes the edge past the real word start and widens the
  // bar (here by the width of the hidden "a"). OCR's "and" sits at the true
  // visible start, so it gives the correct extent.
  function lineSpansFor(box) {
    const ocrRow = spansOnRow(box, 'ocr');
    if (ocrRow.length) return ocrRow;

    // No OCR on this row — use the embedded spans on the box's line.
    if (box.lineId != null) {
      return utbState.boxes.filter(
        (b) => b.page === box.page &&
          (b.type === 'embedded' || b.type === 'ocr') &&
          b.lineId === box.lineId
      );
    }
    // No line association either: fall back to vertical overlap on the page.
    return spansOnRow(box);
  }
